...
test.describe('LocalPDF i18n Translation Tests', () => {
  test('Should switch language and persist in localStorage', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByRole('heading', { level: 2 })).toContainText('Local PDF Tools running 100% securely in your browser');
    
    // Switch to KO
    await page.getByRole('button', { name: 'KO', exact: true }).click();
    
    // Wait for UI text (retrying)
    await expect(page.getByRole('heading', { level: 2 })).toContainText('브라우저에서 100% 안전하게 실행되는 로컬 PDF 도구');
